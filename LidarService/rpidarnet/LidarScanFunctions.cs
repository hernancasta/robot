...
namespace RPLidar
{
    ...
    public partial class Lidar
    {
        ...
        public bool StartScan(ScanMode mode)
        {
            switch (mode)
            {
                ...
                case ScanMode.ExpressExtended:
                    Console.WriteLine("Express extended scan not yet supported.");
                    return false;

                default:
                    Console.WriteLine("Invalid scan mode, could be a bug.");
                    return false;
            }
        }
        ...
        public List<Measurement> GetMeasurements()
        {
            ...
            if (usage > 50)
            {
                Console.WriteLine($"Receive buffer is {usage}% full, should read measurements faster.");
            }
            ...
            {
                case null:
                    Console.WriteLine("No scan mode active.");
                    return null;
                ...
                default:
                    Console.WriteLine($"Invalid scan mode '{activeMode}', it could be a bug");
                    return null;
            }
        }
        ...
        private List<Measurement> GetLegacyMeasurements()
        {
            ...
            for (int i = 0; i < buffer.Length; i += 5)
            {
                ...
                {
                    Console.WriteLine("Receieved invalid scan data (start flags not inverted).");
                    return null;
                }
                ...
                if ((buffer[i + 1] & 1) != 1)
                {
                    Console.WriteLine("Receieved invalid scan data (check bit not set).");
                    return null;
                }
        ...
        private bool ParseExpressLegacyMeasurementsPacket(byte[] buffer, List<Measurement> measurements, out float startAngle)
        {
            ...
            {
                Console.WriteLine("Received invalid scan packet (invalid sync).");
                return false;
            }
            ...
            {
                Console.WriteLine("Received invalid scan packet (invalid checksum).");
                return false;
            }
