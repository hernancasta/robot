    ...
    public partial class Lidar
    {
        ...
        public bool IsOpen
        {
            get
            {
                ...
                {
                    Console.WriteLine($"Error at checking port status. {ex}");
                    return false;
                }
            }
        }
        ...
        public bool Open()
        {
            ...
            {
                Console.WriteLine( $"Error at opening port. {ex}");
        ...
        public bool Close()
        {
            ...
            {
                Console.WriteLine( $"Error at closing port. {ex}");
                return false;
            }
        }
        ...
        private bool GetBytesToRead(out int count)
        {
            ...
            {
                Console.WriteLine($"Error at checking readable bytes count. {ex}");
                count = 0;
                return false;
            }
        }
        ...
        private bool SendCommand(Command command, string commandName)
        {
            ...
            {
                Console.WriteLine($"Error at sending {commandName} command. \n{ex}");
                return false;
            }
        ...
        public bool SetMotorSpeed(UInt16 speed)
        {
            return SendCommand(Command.SetPWMMotor, BitConverter.GetBytes(speed), "setmotorspeed");
        }
        ...
        private bool SendCommand(Command command, byte[] payload, string commandName)
        {
            ...
            {
                Console.WriteLine($"Error at sending command {commandName}.\n{ex}");
                return false;
            }
        ...
        private Descriptor ReadDescriptor(string responseName)
        {
            ...
            while (true)
            {
                ...
                {
                    Console.WriteLine($"Timeout at receiving descriptor for {responseName}.");
                    return null;
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error at receiving descriptor for {responseName}.\n{ex}");
                    return null;
                }
        ...
        private bool CheckDescriptor(Descriptor readDescriptor, Descriptor expectedDescriptor, string responseName)
        {
            ...
            {
                Console.WriteLine($"Expected {responseName} descriptor length {expectedDescriptor.Length}, got {readDescriptor.Length}.");
                return false;
            }

            if (expectedDescriptor.IsSingle != readDescriptor.IsSingle)
            {
                Console.WriteLine($"Expected {responseName} descriptor single to be {expectedDescriptor.IsSingle}, got {readDescriptor.IsSingle}.");
                return false;
            }

            if (expectedDescriptor.DataType != readDescriptor.DataType)
            {
                Console.WriteLine($"Expected {responseName} descriptor data type 0x{expectedDescriptor.DataType:X2}, got 0x{readDescriptor.DataType:X2}.");
                return false;
            }
        ...
        private byte[] ReadResponse(int length, string responseName)
        {
            ...
            {               
                ...
                {
                    Console.WriteLine($"Timeout at receiving data for {responseName}.");
                    return Array.Empty<byte>();
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error at receiving data for {responseName}.\n{ex}");
                    return Array.Empty<byte>();
                }
        ...
        private bool FlushInput()
        {
            ...
            {
                Console.WriteLine($"Error on flushing input buffer.\n{ex}");
                return false;
            }
        }
        ...
        public bool Reset(int waitTime = 700)
        {
            ...
            if (length > 0)
            {
                ...
                Console.WriteLine($"Reset message: {msg}");
            }
        ...
        private byte[] GetConfigurationType(ConfigType configType, byte[] requestPayload, int? expectedResponseLength)
        {
            ...
            {
                Console.WriteLine($"Expected {responseName} response type 0x{configType:X2}, got 0x{responseType:X2}.");
                return Array.Empty<byte>();
            }
